...
with open('cached_network_id.txt') as f:
    # Cached network id loaded from local parameter file.
...
def load_net(target_id, query=None, embedding_dir='image_embeddings', json_dir='net_json'):
    ...
    stylesheet = [{"selector": "[category='regular']","style": {"background-color": "pink",}},
                   {"selector": "[category='central_target']","style": {"background-color": "red",}}]
    ...
    for element in network:
        ...
        if data['category'] == 'central_target':
            continue
